...
#define DEBUG_APPEND_PIPELINE_PADS
...
static void appendPipelineApplicationMessageCallback(GstBus*, GstMessage* message, AppendPipeline* ap)
{
    ap->handleApplicationMessage(message);
}
...
static GstPadProbeReturn appendPipelineAppSrcDataLeaving(GstPad*, GstPadProbeInfo* info, AppendPipeline* self)
{
    if (GST_PAD_PROBE_INFO_TYPE(info) & GST_PAD_PROBE_TYPE_BUFFER) {
        ...
        if (bufferSize > 0)
            self->reportEndOfAppendDataMarkNeeded();
...
static GstPadProbeReturn appendPipelineAppSinkDataEntering(GstPad *, GstPadProbeInfo *info, AppendPipeline* ap)
{
    GstEvent* event = GST_PAD_PROBE_INFO_EVENT(info);
    if (GST_EVENT_TYPE(event) != GST_EVENT_CUSTOM_DOWNSTREAM)
        return GST_PAD_PROBE_OK;
