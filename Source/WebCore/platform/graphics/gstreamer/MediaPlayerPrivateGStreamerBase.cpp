...
bool MediaPlayerPrivateGStreamerBase::handleSyncMessage(GstMessage* message)
{
    ...
    if (GST_MESSAGE_TYPE(message) == GST_MESSAGE_NEED_CONTEXT) {
        ...
        if (!g_strcmp0(contextType, "drm-preferred-decryption-system-id")) {
            ...
            Ref<SharedBuffer> catenatedInitDataChunks = SharedBuffer::create();
            unsigned catenatedInitDataChunksNumber = 0;
            ...
            for (unsigned i = 0; i < streamEncryptionEventsListSize; ++i) {
                ...
                catenatedInitDataChunks->append(reinterpret_cast<const char *>(mapInfo.data), mapInfo.size);
                ++catenatedInitDataChunksNumber;
            ...
            if (catenatedInitDataChunksNumber == 0)
                return false;

            if (catenatedInitDataChunksNumber > 1)
                eventKeySystemIdString = emptyString();

            RunLoop::main().dispatch([this, eventKeySystemIdString, initData = WTFMove(catenatedInitDataChunks)] {
                GST_DEBUG("scheduling keyNeeded event for %s with catenated init datas size of %u", eventKeySystemIdString.utf8().data(), initData->size());
            ...
            m_protectionCondition.waitForSeconds(m_protectionMutex, 4);
